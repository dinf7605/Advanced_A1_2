...
| 8 | 최종 리포트 `.md` 저장 | §6.4 | [ ] |
| 보너스 | 같은 `-date` 재실행 시 캐시 사용 | §9 | [x] |

### 2.3 평가 피드백 반영

| 항목 | 지적 사항 | 대응 | 완료 |
|------|-----------|------|:---:|
| #8 | 지도 API 교체용 추상화 계층 없음 | §5.4 어댑터 인터페이스 + `get_place_provider()` 팩토리 | [x] |
| #17 | `recommended_city` 정규화 로직 없음 | §5.5 `normalize_city()` — 괄호·접미사 제거, 표준명 매핑, 자모 오타 보정 | [x] |
...
## 5.4 지도/장소 제공자 어댑터 (평가 피드백 #8 반영)

> **지적 사항** — 지도 API를 교체할 수 있는 플러그인 인터페이스(추상화 계층)가 없음.
> **보완** — 검색·정규화·예외 처리를 포괄하는 어댑터 인터페이스와 팩토리 진입점 도입.

### 무엇을 어댑터로 뺐는가

제공자마다 실제로 다른 것은 세 가지뿐입니다 — **요청 형태, 응답 구조, 오류 원인.**
타임아웃·HTTP 오류 분기·0건 처리·로그는 어느 제공자든 동일하므로 공통 흐름에 남깁니다.

| 책임 | 위치 |
|------|------|
| 자격증명 확인 | `PlaceProvider.credentials()` / `missing_env()` |
| 요청 구성 (URL·헤더·파라미터) | `PlaceProvider.build_request()` |
| 응답에서 결과 목록 추출 | `PlaceProvider.extract_documents()` |
| 응답 1건 → 공통 스키마 변환 | `PlaceProvider.normalize()` |
| HTTP 오류별 점검 안내 | `PlaceProvider.hint(status)` |
| **위를 제외한 전부** | `search_restaurants()` — 제공자가 바뀌어도 그대로 |
...
def get_place_provider(name=None):
    ...
    ...   # 등록되지 않은 이름이면 사용 가능 목록을 안내하고 종료
```

환경변수 `PLACE_PROVIDER`로 교체합니다 (기본 `kakao`). 코드 수정이 필요 없습니다.

### 검증

| 항목 | 결과 |
|------|------|
| 두 어댑터가 같은 키 집합·타입을 내놓는가 | ✅ `{address, category, lat, lng, name, url}`, 좌표 `float` |
| Naver `title`의 `<b>` 태그 제거 | ✅ `<b>안동</b>찜닭 본점` → `안동찜닭 본점` |
| Naver `mapx`/`mapy` 10⁷ 환산 | ✅ `1287234896` → `128.7234896` |
| 등록되지 않은 제공자 이름 | ✅ 사용 가능 목록 안내 후 exit 1 |
| 자격증명 없는 제공자 선택 | ✅ 건너뛰고 리포트 생성 계속 (`naver_search` / `auth`) |
| `search_restaurants()` 수정 없이 제공자 추가 | ✅ Naver 추가 시 한 줄도 고치지 않음 |

> ⚠️ **`NaverPlaceProvider`는 실호출 미검증입니다.** 자격증명을 발급받지 못해
> 합성 응답으로 정규화만 확인했습니다. 좌표계는 계정·버전에 따라 다를 수 있으므로
> 첫 호출에서 값을 눈으로 확인해야 합니다.

---

## 5.5 추천 도시명 정규화 (평가 피드백 #17 반영)

> **지적 사항** — `recommended_city`의 키워드 정규화(표준화/중앙명사 추출/오타 보정) 없음.
> **보완** — 괄호·접미사 제거, 표준명 맵핑을 도입.

`recommended_city`는 **다음 API의 질의어로 그대로 들어갑니다.** LLM은 같은 도시를
`제주` / `제주도` / `제주특별자치도` / `경주 (경상북도)` / `강 릉` 처럼 다르게 표기하므로,
검색 전에 표준명으로 다듬습니다.

| 단계 | 처리 | 예 |
|:---:|------|-----|
| 1 | 괄호와 내용 제거 | `경주 (경상북도)` → `경주` |
| 2 | 내부 공백 제거 | `강 릉` → `강릉` |
| 3 | 표준명 직접 매핑 (`CITY_ALIASES`) | `제주특별자치도` → `제주` |
| 4 | **표준 도시명이면 중단** | `대구` → `대구` |
| 5 | 행정 접미사 1회 제거 | `경주시` → `경주` |
| 6 | 자모 유사도 오타 보정 | `갱릉` → `강릉` |

### 왜 4단계가 필요한가

접미사 목록에 `구`가 있어 **`대구` → `대`** 가 되어 버립니다.
그래서 (a) 이미 표준 도시명이면 접미사 제거를 시도하지 않고,
(b) 제거 결과가 두 글자 미만이면 원본을 유지합니다.

### 왜 자모로 분해하는가

한글 도시명은 두 글자가 많아 **글자 단위 비교로는 오타를 못 잡습니다.**
`갱릉` vs `강릉`은 2자 중 1자 일치로 유사도 0.5입니다.
자모로 풀면 `ㄱㅏㅇㄹㅡㅇ` / `ㄱㅐㅇㄹㅡㅇ` 로 6개 중 5개 일치, 0.83이 되어
기준값 `CITY_MATCH_CUTOFF = 0.8`을 넘깁니다.

### 추적성

원본과 달라지면 **무엇을 왜 바꿨는지** 로그와 결과 JSON에 남깁니다.
조용히 바꾸면 검색 결과가 이상할 때 원인을 찾을 수 없습니다.
...
목록에 없는 지명은 **그대로 두고 검색합니다.** 표준 목록에 없다고 버리면
덜 알려진 여행지를 추천받았을 때 검색조차 못 합니다.
...
> ✅ **9종 전부 촬영 완료 (2026-08-18)** — `docs/screenshots/`, README에 반영됨.
> 촬영 후 실제 키가 찍히지 않았는지 전 장 확인함.
...
| `05_report.png` | 최종 리포트 (맛집 표가 채워진 경우) |
| `05_report2.png` | 최종 리포트 (맛집 '데이터 없음' + 오류 요약) |
| `06_error_kakao.png` | **맛집 실패 후에도 리포트가 생성된 로그** (T7). 촬영 중 503 재시도까지 함께 잡혀 오류 처리 3종이 한 장에 담김 |
